...
if __name__ == "__main__":
    ...
    M = 3             # 隱藏層神經元數
    T = 50000         # 每次訓練迭代次數
    trials = 500      # 每個 η 試 500 次
    layer_n = 3       # 三層：input → hidden → output

    # 3. 測試多組 η
    eta_list = np.array([0.001, 0.01, 0.1, 1, 10])
    err_avg = np.zeros(eta_list.shape[0])   # 儲存平均錯誤
    ...
    for i, eta_val in enumerate(eta_list):
        ...
        for t in range(trials):
            # 訓練神經網路
            w_list = nnet_bp(train_x, train_y, layer=layer_n, dim=dim, eta=eta_val, r=r, T=T)
        ...
        print(f"eta = {eta_val}, average error = {err_avg[i]}")

    # 找出最小平均錯誤的 eta
    best_eta = eta_list[np.argmin(err_avg)]
