...
if __name__ == "__main__":
    ...
    eta = 0.1
    r = 0.1
    M_list = np.array([1, 6, 11, 16, 21])  # different hidden layer sizes to test
    layer_n = 3                            # input -> hidden -> output
    T = 50000                              # training iterations
    trials = 10                            # number of experiments per setting

    # 3. Prepare to record average errors
    err_avg = np.zeros(M_list.shape[0])

    # 4. Test each hidden layer size
    for i, hidden_size in enumerate(M_list):
        ...
        print(f"Hidden size {hidden_size}: average error = {err_avg[i]:.4f}")

    # 5. Find the best hidden layer size
    best_idx = np.argmin(err_avg)
    print(f"\nBest hidden size = {M_list[best_idx]}, with error {err_avg[best_idx]:.4f}")
